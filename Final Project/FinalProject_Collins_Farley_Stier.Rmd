...
We can see that there are data from 2814 of the 3006 counties in the US, with a fairly narrow distribution of the number of counties of each rural-urban type. The distributions in the quantitative variables are pretty consistent, which we also saw in the box plot above, meaning that standardization might not make a huge difference. Let's go ahead and make a standardized version of the data set anyway, though.
...
```{r}
# continuous metric data only (standard log metric)
stlm <- stlogdata[7:15]

CSQPlot(stlm[7:15], label="COVID-19 Data")
```

First off, we note that the metric variables in our data do not follow a multivariate normal distribution. This is not an assumption for PCA but is of note because we will not be able to use parallel analysis to determine which components to keep.

```{r}
corrplot.mixed(round(cor(stlm), 2), lower.col = "black", upper = "ellipse", tl.col = "black", number.cex = .3, order = "hclust", tl.pos = "lt", tl.cex = .25)
```

PCA should work well because there are many variables highly correlated with other variables. For instance, the correlation between the log of the unemployment rate and the labor force as a percent is -.065, the correlation between the log of the median household income and percent poverty is -0.88, and the correlation between the percent of COVID-19 cases and the percent of COVID-19 deaths is 0.47. There appear to be underlying trends about the counties (about beliefs about COVID-19, about wealth/education, etc) that could be summarized in linear combinations of the 19 metric variables we have currently.
...
# Conclusions and Discussion (todo)

# Points for Further Analysis (todo)
