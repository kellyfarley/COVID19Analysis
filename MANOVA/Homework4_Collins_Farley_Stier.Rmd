...
That looks a lot better, especially at the low values. At higher values, the points start to stray outside the 95% CI, but it looks acceptable overall.
...
```{r, eval=FALSE}
(mrppout <- mrpp(raw[,c("newNever", "newSometimes", "newAlways")], raw$rural_urban_code))
```
...
delta 0.2373 0.2383   0.2187
n     1077   899      1165  

Chance corrected within-group agreement A: 0.06974 
Based on observed delta 0.2307 and expected delta 0.248 
...
And those results look pretty good! The p-value is 0.001 according to the above output, so MRPP tells us that the groups cluster well enough and are distinct by the procedure's permutation-delta method.