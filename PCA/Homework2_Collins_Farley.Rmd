...
author: "Kelly Farley + Evan Collins"
...
db <- subset(db, select=-c(16,17)) # exclude categorical rural code
dim(db)

raw2 <- read.csv("/Users/Evan/Desktop/s&ds 563/covid_and_demographics_kelly.csv")
db2 <- subset(raw2, select=c(4:24)) # exclude ID columns except FIPS
db2 <- subset(db2, select=-c(17)) # exclude categorical rural code
dim(db2)
```
